 ...
 * 1. User guarantees *v_ptr points to the correct L2BUFF mapped virtual space starting address
 ...
 * @param [in] device_id:  unsigned shaping, device id, this value is not used in offline scenarios
 ...
 * @param [in] byte_count:  the length of the program to be loaded
 * @param [out] v_ptr:  the start address of L2BUF is used as the input. After the load is completed, the address of
 ...
 * @return DRV_ERROR_NONE : success
 */
DLLEXPORT DV_OFF_ONLINE DVresult drvLoadProgram(
    DVdevice device_id, void *program, unsigned int offset, size_t byte_count, void **v_ptr);
...
/**
* @ingroup driver
* @brief Get DQS mbuf handle for ISP enque
* @attention null
* @param [in] Mbuf *mbuf: Mbuf addr
* @param [out] uint64_t *handle: DQS mbuf handle
* @return   0 for success, others for fail
*/
DLLEXPORT drvError_t halGetMbufhandleforEnque(Mbuf *mbuf,  uint64_t *handle);
